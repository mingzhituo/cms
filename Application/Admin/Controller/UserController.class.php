...
class UserController extends BaseController {
    ...
    public function detail(){
        if(IS_POST){
            ...
            if($password != '' && $password != $password2){
                $this->ajaxReturn(array('status'=>-1,'msg'=>'两次输入密码不同'));
            }
            ...
            $row = M('users')->where(array('user_id'=>$_POST['user_id']))->save($_POST);
            if($row){
                $this->ajaxReturn(array('status'=>1,'msg'=>'修改成功'));
            }else{
                $this->ajaxReturn(array('status'=>-2,'msg'=>'未作内容修改或修改失败'));
            }
        }else{
            $uid = I('get.id');
    ...
    public function delete(){
        ...
        if($row){
            $this->ajaxReturn(array('status'=>1,'msg'=>'成功删除会员'));
        }else{
    ...
    public function level(){
        $level_id = I('GET.level_id',0);
        $act = $level_id>0?'edit':'add';
        $this->assign('act',$act);
    	$level_info = array();
    	if($level_id){
    ...
    public function doSendMessage()
    {
        ...
        if ($type == 1) {
            //全体用户系统消息
            $message['type'] = 1;
            $re = M('Message')->data($message)->add();
        } else {
            //个体消息
            $message['type'] = 0;
            if (!empty($users)) {
                $re = $create_message_id = M('Message')->data($message)->add();
