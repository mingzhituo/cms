...
class CouponController extends BaseController {
    ...
    public function coupon_info(){
        ...
        $this->assign('act',$act);           
        if ($id>0) {
            $coupon = M('coupon')->where(array('id'=>$id))->find();
     ...
     * 优惠券类型处理
     */
    public function couponInfoHandle(){
        $model = M("coupon");
        $data = I('post.');
        ...
        if($data['send_start_time'] > $data['send_end_time']){
            $this->ajaxReturn(array('status'=>-1,'msg'=>'发放日期填写有误!'));
        }     
        if ($data['act'] == 'add') {  
            $data['add_time'] = time();          
            ...
            $row = M('coupon')->where(array('id'=>$data['id']))->delete();  
            if ($row) {
                //删除此类型下的优惠券
                M('coupon_list')->where(array('cid'=>$data['id']))->delete();
    ...
    * 优惠券发放处理
    */    
    public function makeCouponHandle(){
        //查询是否存在优惠券
        $cid = I('post.cid');
        $type = I('post.type');
        $data = M('coupon')->where(array('id'=>$cid))->find(); 
        ...
        $remain<=0 && $this->ajaxReturn(array('status'=>0,'msg'=>$data['name'].'已经发放完了'));
        !$data && $this->ajaxReturn(array('status'=>-1,'msg'=>'优惠券类型不存在'));
        $type != 4 && $this->ajaxReturn(array('status'=>-2,'msg'=>'该优惠券类型不支持发放'));
        if(IS_POST){
            $num  = I('post.num');
            $num>$remain && $this->ajaxReturn(array('status'=>-3,'msg'=>$data['name'].'发放量不够了'));
            !$num > 0 && $this->ajaxReturn(array('status'=>-4,'msg'=>'发放数量不能小于0'));
            $add['cid'] = $cid;
            $add['type'] = $type;
            $add['send_time'] = time();
            for($i=0;$i<$num; $i++){
            ...
            $this->ajaxReturn(array('status'=>1,'msg'=>'发放成功！'));
        }
    }   
