    ...
    <div class="panel panel-default">
        ...
        <div class="page-container">        
            <div class="navbar navbar-default">
				<form action="<?php echo U('Admin/Article/articleList');?>" id="search-form2" class="navbar-form form-inline l" method="post" style="width:32%">
					<div class="formControls">
						...
						<span class="select-box" style="width:50%;">
                            ...
                            <?php if(is_array($cat_select)): foreach($cat_select as $key=>$vo): ?><option value="<?php echo ($vo["cat_id"]); ?>" <?php if($vo['parent_id'] == 0): ?>disabled<?php endif; ?> <?php if($vo['cat_id'] == $cat_id): ?>selected<?php endif; ?>> <?php if($vo['level'] != 0): ?>|<?php endif; echo str_repeat('-', $vo['level']*4); echo ($vo["cat_name"]); ?></option><?php endforeach; endif; ?> 
                        </select>
            	...
            	<span class="r mt-10 mr-20"> <a href="javascript:;" onclick="admin_add('添加文章','<?php echo U("admin/Article/article",array('catid'=>$cat_id));?>','1000','800')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加文章</a></span> 
            </div>
            <table class="table table-border table-bordered radius table-hover" id="list-table" style="border-collapse:collapse;">
                ...
                <tbody>
                    <?php if(is_array($list)): foreach($list as $key=>$vo): ?><tr class="text-c">
                        <td class="va-m" style="width:200px;"><?php echo ($vo["title"]); ?></td>
                        <td class="va-m"><?php echo ($vo["cat_id"]); ?></td>
                        <td class="va-m" style="width:200px;"><?php echo (mb_substr($vo["description"],0,40,"utf-8")); ?>......</td>
                        <td class="va-m"><?php if($vo["is_open"] == 1): ?>是<?php else: ?>否<?php endif; ?></td>
                        <td class="va-m"><img style="height:150px;" src="<?php echo ($vo["thumb"]); ?>" alt=""></td>
                        <td class="va-m"><?php echo (date('Y-m-d H:i:s',$vo["publish_time"])); ?></td>
                        <td>
                            <a title="编辑" href="javascript:;" onclick="admin_edit('文章编辑','<?php echo U("admin/Article/article/article_id/".$vo['article_id']);?>','1','1000','800')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont btn btn-info radius">&#xe6df;</i></a>
                            <a title="删除" href="javascript:;" onclick="admin_del('<?php echo U("admin/Article/aticleHandle/");?>',this,<?php echo ($vo['article_id']); ?>)" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont btn btn-danger radius">&#xe6e2;</i></a>
                        </td>
