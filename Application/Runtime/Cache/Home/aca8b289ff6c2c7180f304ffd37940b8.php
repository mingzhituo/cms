 ...
 <head>
   ...
   <link rel="stylesheet" href="/Public/Home/css/style8.css">
    ...
    <div class="auto nei">
      ...
      <p class="weizhi"> 当前位置：
        <a href="/">首页</a>&gt;&gt;<a href="">麻将文化</a>
      </p>


      <div class="fu_right news-index shipin-index">
        <div class="nei_left table2">

          <h3><?php echo ($on["title"]); ?></h3>
          <ul class="news-index-ul">
            <li>发布时间:<?php echo (date("Y-m月-d日 h:i:s",$on["Publish_time"])); ?></li>
            <li>文章来源:<?php echo ($config2["company_name"]); ?></li>
           <!--  <li>作者:闪念网络</li> -->
            <li>阅读量:<?php echo ($on["click"]); ?></li>
          </ul>
          <!-- 百度分享 -->
          ...
          <div class="">
               <p><?php echo (htmlspecialchars_decode($on['content'])); ?></p>
          </div>
          <ul class="news-index-ul page">
            <!-- 上一篇 -->
            <?php if($prev == ''): ?><li><a href="#" title="">
                  上一篇:暂无</a></li>
            <?php else: ?>
            <li>上一篇:<a href="<?php echo U(''.CONTROLLER_NAME.'/page',array('id'=>$prev['article_id'],'catid'=>$prev['cat_id']));?>"><?php echo ($prev["title"]); ?></a></li><?php endif; ?>
            <!-- 下一篇 -->
              <?php if($next == ''): ?><li><a href="#" title="">
                  下一篇:暂无</a></li>
            <?php else: ?>
            <li>下一篇:<a href="<?php echo U(''.CONTROLLER_NAME.'/page',array('id'=>$next['article_id'],'catid'=>$next['cat_id']));?>"><?php echo ($next["title"]); ?></a></li><?php endif; ?>
          </ul>
        </div>
